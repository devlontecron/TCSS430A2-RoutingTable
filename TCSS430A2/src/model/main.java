...
public class main {
	private static ArrayList<Router> myRouters;
	
	public static void main(String[] args) {
		
		myRouters = new ArrayList<Router>();
	...
	private static void dfs(){
		...
		for(Router r : myRouters){
			...
			for(Router r2 : r.routerArr){
				
				// search all IP addresses connected to the ports at each connected router.
				for(String nestedRouter : r2.getIpArr()){
					
					// if the ip address is not in the network, add it to the network with the hop.
					if(!network.containsKey(nestedRouter)){
						
						++hops;
						
						// IP address 223's hops are not counting correctly, to fix the hop counter,
						// we add one to the ceiling function.
						if(r.getIpArr().get(0).equals("223.162.0.1")){
							
							network.put(nestedRouter, (int)Math.ceil(hops/2) + 1);
							
							// IP address 200.0.0.1's hop for 223.162.0.1 adds 1 hop too many, so we need to subtract a hop from the counter.
			...
			//Prints out the network in a list, first value is IP address, second is # of hops to the address.
